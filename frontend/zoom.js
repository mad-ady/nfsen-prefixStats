...
// Global constant
var cURLBase = "zoom.pl?action=zoom";
...
function zoomGraphObj(zoomGraphName) {
	...
	gUrlObj = new urlObj(window.location.href);
...
*/
	//HARDCODED position for the overlay image generated by the selection
	//the position depends on what elements are displayed and where they are placed.
	divObject.style.left = 80;
	divObject.style.top = 42;
	divObject.style.width = 670;
	divObject.style.height = 282;
...
function onMouseUpEvent(e) {
	...
	if ((gMouseObj.rightButtonPressed()) && (insideZoomBox())) {
		...
		var rrd = gUrlObj.getUrlParameterValue("rrd");
		var filename = gUrlObj.getUrlParameterValue("filename");

		open(urlBase + "&graph_start=" + newGraphStart + "&graph_end=" + newGraphEnd + "&title=" + title + "&rrd=" + rrd + "&filename=" + filename, "_self");
	}
